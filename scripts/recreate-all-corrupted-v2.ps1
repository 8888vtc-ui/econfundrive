# Script V2 pour RECREER TOUS les fichiers corrompus (détection améliorée)
$ErrorActionPreference = "Stop"

Write-Host "RECREATION TOUS FICHIERS CORROMPUS V2" -ForegroundColor Cyan
Write-Host ""
...
foreach ($file in $pageFiles) {
    try {
        ...
        # Problème 1: Chaînes non terminées dans keywords (tous patterns)
        if ($content -match "keywords = \[.*'[^']*'$" -or 
            ...
            ($content -match "keywords = \[" -and $content -match "'[^']*'`r?`n\s*'Azur -->") -or
            $content -match "keywords = \[.*'[^']*'`r?`n\s*'Azur'`r?`n\s*'\]" -or
            $content -match "keywords = \[.*'[^']*'`r?`n\s*'Azur'`r?`n\s*'\]" -or
            $content -match "keywords = \[.*'[^']*'`r?`n\s*'Azur'`r?`n\s*'\]") {
            $isCorrupted = $true
        }
        ...
        # Problème 3: Encodage corrompu dans keywords
        if ($content -match "const keywords = \[.*privÃ©" -or 
            $content -match "const keywords = \[.*CÃ´te" -or 
            $content -match "const keywords = \[.*dAzur" -and $content -notmatch "d'Azur") {
            $isCorrupted = $true
        }
        
        # Problème 4: Chaînes avec apostrophe mal échappée
        if ($content -match "keywords = \[.*'[^']*'`r?`n\s*'Azur'`r?`n\s*'\]" -or
            $content -match "keywords = \[.*'[^']*'`r?`n\s*'Azur'`r?`n\s*'\]") {
            $isCorrupted = $true
        }
        
        # Problème 5: Vérifier si le fichier peut être parsé (test basique)
        try {
            # Essayer de trouver un pattern keywords valide
            if ($content -match "keywords\s*=\s*\[") {
                $keywordsSection = [regex]::Match($content, "keywords\s*=\s*\[([^\]]+)\]", [System.Text.RegularExpressions.RegexOptions]::Singleline)
                if ($keywordsSection.Success) {
                    $keywordsContent = $keywordsSection.Groups[1].Value
                    # Vérifier s'il y a des chaînes non fermées
                    $openQuotes = ([regex]::Matches($keywordsContent, "'")).Count
        ...
        if ($isCorrupted) {
            $corruptedFiles += $file
            Write-Host "  CORROMPU: $($file.FullName.Replace($PWD.Path + '\', ''))" -ForegroundColor Yellow
        }
    } catch {
